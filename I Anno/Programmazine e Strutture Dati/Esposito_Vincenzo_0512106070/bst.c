...
list elementiCompresi(BST T, item val1, item val2)
{
     list l=newList();
     item max, min;
...
list RicercaRicorsiva(BST T, list l, item min, item max)            //funzione ricorsiva 
{
                     if (T == NULL)  return l;

                     if(minore(getItem(T),min)&&minore(getItem(T),max))        //controlla se il valore e' compreso tra i due elementi
                     {
                                                                       insertList(l,sizeList(1),getItem(T);
                                                                       l=RicercaRicorsiva(figlioSX(T),l,min,max);
                                                                       l=RicercaRicorsiva(figlioDX(T),l,min,max);
                     }
                     else
                     {     
                     if(minore(getItem(T),min)
                     {
                                              l=RicercaRicorsiva(figlioDX(T),l,min,max);
                     }
                     else
...
BST cancMaggiori(BST T, item val1)
{     
      item val1;
      
      if (T == NULL) return T;
      
      item radice=getItem(T);
      item fdx=getItem(figlioDX(T));
      item fsx=getItem(figlioSX(T));
                                                                           
      if(!minore(radice,val1))                                              //controlla se e' maggiore del valore dato lo cancella 
      {deleteNode(T,radice);
      
      T=cancMaggiori(T,fdx);
      T=cancMaggiori(T,fsx);
      }
}
